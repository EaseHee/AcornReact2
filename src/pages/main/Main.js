...
const handleLogout = async () => {
    ...
    useEffect(() => {
        const successCallback = (position) => {
            ...
            console.log("위치 불러오기 성공", position.coords.latitude, position.coords.longitude);
        }
                ...
                <Flex justify="space-between" wrap="wrap" gap={4}>
                    {items.map((_, index) => ( /* List<Eateries>.map */
                        <Box
                            ...
                            borderRadius="md"
                        >
                            <Maincard no={(index%5)+1}></Maincard>{/* 음식점 상세 정보 페이지 <- eateriesNo 전달 */}
                        </Box>
                    ))}
